...
import fontUrl from '@openfonts/barlow-condensed_all/files/barlow-condensed-all-400.woff2';
import iconFontUrl from '@mdi/font/fonts/materialdesignicons-webfont.woff2';
...
const blobCaches = new Map();

// load font file and encode as data url
// cannot use URL.createObjectURL since blobs aren't supported by SVG (or not allowed)
function loadFont(url) {
  if (url.startsWith('data:')) {
    return Promise.resolve(url);
  }
  if (blobCaches.has(url)) {
    return blobCaches.get(url);
  }
  const objectUrl = fetch(url).then(r => r.blob()).then(blob => new Promise((resolve) => {
    const fr = new FileReader();
    fr.onload = () => resolve(fr.result);
    fr.readAsDataURL(blob);
  }));
  blobCaches.set(url, objectUrl);
  return objectUrl;
}
...
export async function svg2url(svgElement, options = {}) {
  ...
  if (includeIconFont) {
    const iconFont = await loadFont(iconFontUrl);
    copy.insertAdjacentHTML('afterbegin', `<style>
...
export function downloadCSV(content, title) {
  const csvUrl = jsoncsv2url(content);
